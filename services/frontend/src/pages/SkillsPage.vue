...
async function loadSkills() {
  skills.value = await api<any[]>("/api/v1/assistant/skills");
}
...
async function loadOptions() {
  ...
  options.knowledgeBases = knowledgeBases.filter((item) => item.status === undefined || item.status === "active");
  options.tools = (connectorResult.items || []).flatMap((connector: any) =>
    (connector.tools || []).filter((tool: any) =>
      (tool.status === undefined || tool.status === "active") && tool.annotations?.readOnlyHint === true,
    )
      .map((tool: any) => ({ ...tool, label: `${connector.name} / ${tool.title || tool.tool_name}` })),
  );
  options.agents = agents.filter((item) => item.status === undefined || item.status === "active");
  options.departments = departments.filter((item) => item.status === undefined || item.status === 1);
}
...
async function openSkill(item: any) {
  try {
    selected.value = await api<any>(`/api/v1/assistant/skills/${item.id}`);
...
async function save() {
  ...
  try {
    ...
    const saved = form.id
      ? await api<any>(`/api/v1/assistant/skills/${form.id}`, { method: "PUT", body })
      : await api<any>("/api/v1/assistant/skills", { method: "POST", body });
    await loadSkills();
...
async function disable() {
  ...
  busy.value = true;
  try {
    const saved = await api<any>(`/api/v1/assistant/skills/${form.id}?version=${form.version}`, { method: "DELETE" });
    await loadSkills();
    selected.value = saved;
  ...
  <div class="skills-layout">
    ...
    <form v-if="editing" class="card form-stack skill-form" @submit.prevent="save">
      ...
      <div class="binding-grid">
        <fieldset><legend>部门</legend><label v-for="item in options.departments" :key="item.id" class="check"><input v-model="form.department_ids" type="checkbox" :value="item.id">{{ item.name }}</label><small v-if="!options.departments.length">暂无可选项</small></fieldset>
        <fieldset><legend>知识库</legend><label v-for="item in options.knowledgeBases" :key="item.id" class="check"><input v-model="form.knowledge_base_ids" type="checkbox" :value="item.id">{{ item.name }}</label><small v-if="!options.knowledgeBases.length">暂无可选项</small></fieldset>
        <fieldset><legend>MCP 只读工具</legend><label v-for="item in options.tools" :key="item.id" class="check"><input v-model="form.tool_ids" type="checkbox" :value="item.id">{{ item.label }}</label><small v-if="!options.tools.length">暂无可选项</small></fieldset>
        <fieldset><legend>智能体</legend><label v-for="item in options.agents" :key="item.id" class="check"><input v-model="form.agent_ids" type="checkbox" :value="item.id">{{ item.name }}</label><small v-if="!options.agents.length">暂无可选项</small></fieldset>
      </div>
