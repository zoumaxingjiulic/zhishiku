...
<template>
  ...
  <div class="page-toolbar"><div class="actions"><button class="secondary" @click="openDepartment">＋ 新建部门</button><button class="primary" @click="openUser()">＋ 创建账号</button></div></div>
  <BaseCard class="content-card" padding="none"><div v-if="loading && !users.length" class="page-loading"><BaseSkeleton height="48px" /><BaseSkeleton height="48px" /><BaseSkeleton height="48px" /></div><BaseEmptyState v-else-if="loadError" title="用户目录加载失败" :description="loadError" role="alert"><template #action><BaseButton variant="secondary" @click="load">重试</BaseButton></template></BaseEmptyState><div v-else-if="users.length" class="table-wrap"><table><thead><tr><th>账号</th><th>姓名</th><th>部门</th><th>状态</th><th>最后登录</th><th>操作</th></tr></thead><tbody><tr v-for="user in users" :key="user.id"><td>{{ user.username }}</td><td>{{ user.display_name }}</td><td><span class="badge" :class="{'admin-dept':user.department_code==='PLATFORM_ADMIN'}">{{ user.department_name||"未分配" }}</span></td><td><StatusBadge :status="user.status"/></td><td>{{ formatDate(user.last_login_at) }}</td><td><div class="actions table-actions"><button class="secondary" @click="openUser(user)">编辑</button><button class="secondary" @click="reset(user)">重置密码</button><button :class="user.status?'danger':'secondary'" @click="setStatus(user)">{{user.status?'停用':'启用'}}</button><button class="danger" @click="remove(user)">删除</button></div></td></tr></tbody></table></div><BaseEmptyState v-else title="暂无账号" description="创建企业账号后，可按部门授予数据访问权限。" /></BaseCard>
