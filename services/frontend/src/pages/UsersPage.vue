...
async function load() { [departments.value, users.value] = await Promise.all([api<DepartmentDto[]>("/api/v1/departments"), api<UserDto[]>("/api/v1/users")]); }
onMounted(load);
...
function closeTemporaryPassword() { clearTemporaryPassword(); modal.value=""; }
...
async function copyPassword(){if(!temporaryPasswordNotice.value)return;await navigator.clipboard.writeText(temporaryPasswordNotice.value.temporaryPassword);emit("toast","临时密码已复制");}
</script>
<template>
  <div class="section-head"><div><h2>账号与部门权限</h2><p>权限完全由所属部门决定；平台管理员部门拥有全局管理权限</p></div><div class="actions"><button class="secondary" @click="openDepartment">＋ 新建部门</button><button class="primary" @click="openUser()">＋ 创建账号</button></div></div>
  ...
  <AppModal v-if="modal==='password' && temporaryPasswordNotice" title="临时密码（仅显示一次）" @close="closeTemporaryPassword"><div class="temporary-password"><p>账号 <strong>{{temporaryPasswordNotice.username}}</strong> 的临时密码：</p><div class="password-once"><code>{{temporaryPasswordNotice.temporaryPassword}}</code><button class="secondary" @click="copyPassword">复制密码</button></div><button class="primary wide" @click="closeTemporaryPassword">我已保存，关闭</button></div></AppModal>
</template>
