...
def load_knowledge_base_permission(user: dict, knowledge_base_id: int, manage: bool = False) -> dict:
    """Compatibility entry point for domains migrated in later tasks."""
    with UnitOfWork() as uow:
        return KnowledgeService(uow).require_knowledge_base(user, knowledge_base_id, manage)


def load_accessible_knowledge_base_ids(user: dict) -> list[int]:
    """Compatibility entry point for domains migrated in later tasks."""
    with UnitOfWork() as uow:
        return KnowledgeService(uow).accessible_knowledge_base_ids(user)


def load_folder_document_ids(
    knowledge_base_id: int,
    folder_id: int,
    include_subfolders: bool,
) -> list[int]:
    """Compatibility entry point for chat routes migrated in later tasks."""
    with UnitOfWork() as uow:
        return KnowledgeService(uow).folder_document_ids(
            knowledge_base_id,
            folder_id,
            include_subfolders,
        )
...
def folder_for_cursor(cursor, folder_id: int, knowledge_base_id: int) -> dict:
    repository = KnowledgeRepository(cursor)
    folder = repository.get_active_folder(folder_id)
    if not folder or folder["knowledge_base_id"] != knowledge_base_id:
        raise ValidationError("文件夹不存在或不属于当前知识库")
    return folder


def folder_descendants_for_cursor(cursor, folder_id: int, include_self: bool = True) -> list[int]:
    ids = KnowledgeRepository(cursor).folder_descendant_ids(folder_id)
